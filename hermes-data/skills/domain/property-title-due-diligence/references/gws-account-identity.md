...
4. At the upload moment the request resolved to Prakash ([REDACTED-TID]), so the
   subprocess ran with HERMES_SESSION_USER_ID=[REDACTED-TID] → canonical_uid →
   `psingh-[REDACTED-TID]` → `get_token(uid, 'google-draas')` returned **Prakash's**
   OAuth token. Files went into his TMP folder
   (155SBzMDhM5pebj3wLz8o_9310ZsFwqh6).
5. The same session on a later turn resolved to [REDACTED-TID] (Nishant) — the
...
- The vault stores tokens per `(canonical_uid, service)`:
  `ndr-[REDACTED-TID]/google-draas` = ndr's token, `psingh-[REDACTED-TID]/google-draas`
