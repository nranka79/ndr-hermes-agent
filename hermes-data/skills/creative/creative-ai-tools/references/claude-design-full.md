...
## DRAAS Investment Document Convention

When designing investment/opportunity documents for DRAAS (real estate, land acquisition, co-investment opportunities), follow this visual system unless the user specifies otherwise:

- **Background**: `#1a1a2e` (deep navy)
- **Text**: `#f0ede6` (warm off-white)
- **Accent / highlight**: `#c9a84c` (gold)
- **Surface**: `#22243a` (card background)
- **Font**: Inter (Google Fonts) with `DejaVu Sans` fallback
- **Typography**: 9px 4-letter-spacing uppercase labels; 22px bold stat values; 14px body
- **Layout patterns**:
  - 4-column stats row with gold labels
  - Split cards for dual-option comparisons (Option 1 preferred / Option 2 fallback)
  - Numbers grid with 3 columns for financial summary
  - Q&A blocks with gold `Q:` badge prefix
- **Print-to-PDF workflow**: If no PDF generation tool is available, deliver self-contained HTML + CSS; user prints to PDF. Include `print-color-adjust: exact` and `-webkit-print-color-adjust: exact` in CSS.

## Source-Document Reading Protocol

Before designing, always extract the full content of any PDF the user shares — use `pdftotext` (CLI) first, then fall back to `pymupdf` (Python). Never rely on the filename or the user's description alone — the actual document content determines the layout, numbers, and language. Extract and confirm before building.
...
- **DRAAS documents**: Never name "Prestige" or any specific developer brand publicly — always use "Grade A Developer" or "partner developer" in investor-facing documents. The user's internal naming must never appear in client-facing output.
