...
- **Tunnel router is OPERATIONAL.** Residential-listed domains (MagicBricks, 99acres, rera.karnataka.gov.in, google/bing/duckduckgo, etc.) egress through a connected residential node; everything else defaults to `direct` (VPS IP). Verify with a **listed** echo endpoint, e.g. `curl -s --socks5-hostname hermes-utilities:1000 https://ifconfig.me` → residential node IP. **DO NOT verify with `httpbin.org/ip` or `api.ipify.org`** — those are NOT in the residential list, so they correctly show the VPS IP by design. Seeing the VPS IP on a non-listed echo endpoint is EXPECTED, not a sign the tunnel is broken. Test against the actual portal domain (MagicBricks/99acres) or `ifconfig.me`.
