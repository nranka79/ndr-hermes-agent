...
- **Kelsa workflow**: See `references/kelsa-pipeline-invoice-exploration.md` for the full pattern of exploring Kelsa accounts, pipelines, leads, and tasks via MCP with a token-authenticated HTTP server. For the complete Kelsa-Read MCP tool inventory, critical pitfalls (URL construction, search syntax, indexing), and account/pipeline ID reference, see `references/kelsa-read-mcp-adhoc.md`.
