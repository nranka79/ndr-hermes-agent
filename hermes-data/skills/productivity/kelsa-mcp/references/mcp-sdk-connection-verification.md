...
Then the token file is root-owned and the `hermes` user (UID 10000) can't read it. Verification:

```bash
ls -la /data/hermes/mcp-tokens/Kelsa-Read.json
# -rw------- 1 root root 264 ...   ← hermes can't read this
```

**Resolution**: `hermes mcp login Kelsa-Read` in a PTY terminal to re-run the OAuth flow, which rewrites the files with hermes ownership.
