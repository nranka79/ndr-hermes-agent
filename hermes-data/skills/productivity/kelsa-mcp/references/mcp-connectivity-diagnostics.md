...
```bash
ls -la /data/hermes/mcp-tokens/Kelsa-Read*
```

- `Kelsa-Read.json` — OAuth tokens (should be `hermes:hermes`, not `root:root`)
- `Kelsa-Read.client.json` — OAuth client registration
- `Kelsa-Read.meta.json` — OAuth server metadata
