...
**A) On the cron job definition:** Set the `owner` field to the canonical vault user ID (e.g. Nishant's `7449813913`) so `HERMES_CRON_JOB_OWNER_ID` gets populated. This is the permanent fix and allows the job to run unattended.

**B) Running manually:** Pass `HERMES_SESSION_USER_ID` on the command line:
```bash
HERMES_SESSION_USER_ID=7449813913 python3 /data/hermes/scripts/daily_meta_import.py
```
...
- The cron job owner is `sales1.blr` (Telegram ID `8717455402`), not Nishant
