...
Then the process hangs at "completing flow" for minutes. Checking the token directory shows no `Kelsa-Read.json` was created.
...
2. Check if token was actually exchanged:
   ```bash
   ls -la /data/hermes/mcp-tokens/Kelsa-Read.json
   ```
3. If the file exists (owned hermes:hermes), the token was exchanged despite the hang. Restart the gateway to pick it up.
4. If the file does NOT exist, the auth code was consumed but token exchange failed. Clean up and restart:
   ```bash
   echo "y" | hermes mcp remove Kelsa-Read
   ```
...
- **Diagnose the token directory first** — Before running the OAuth flow, check which failure mode you're in:
  ```bash
  ls -la /data/hermes/mcp-tokens/
  ```
  - Files present, owned `hermes:hermes` → expired token, re-auth needed
  - Files present, owned `root:root` → permissions broken, re-auth rewrites them
  - Directory empty or no files → OAuth never completed, remove + re-auth needed
  All three run the same fix below.
