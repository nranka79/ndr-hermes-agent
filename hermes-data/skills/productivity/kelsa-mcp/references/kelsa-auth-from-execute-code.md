...
## 🔐 Session context: fixing "No session user context" errors in terminal()

When `kelsa_list_tools` or `kelsa_call_tool` fail with `No session user context -- cannot determine which user's Kelsa token to use`, it means the gateway's `HERMES_SESSION_USER_ID` env var isn't set (common in mid-session terminal scripts). The fix: set it explicitly before importing from kelsa_auth, using the user's canonical Telegram/channel ID.

Find the user's ID by checking `/data/hermes/cron/jobs.json` — look at any job's `origin.chat_id` field, or the job's `owner` field (format `ndr-7449813913` — the suffix is the ID).

```python
import os
os.environ['HERMES_SESSION_USER_ID'] = '7449813913'  # Nishant's ID
os.environ['GWS_VAULT_SOCKET'] = '/run/gws-vault/vault.sock'
# THEN import kelsa_auth / get_valid_access_token
```

Without this, `get_valid_access_token()` raises `ValueError: No session user context`. Setting the env var before the import resolves the identity for vault lookups.

Now, note the functions `has_token()` and `get_valid_access_token()` take **zero arguments** — they read `_session_telegram_id()` internally. The old reference code in prior versions passed `telegram_id` as an arg; that no longer works.
...
# Note: has_token() and get_valid_access_token() take zero arguments —
# they read session identity internally from HERMES_SESSION_USER_ID.
if has_token():
    try:
...
# Note: get_auth_url() takes zero args — reads identity from HERMES_SESSION_USER_ID.
url = get_auth_url()
...
# Both telegram_id and code_verifier are extracted from state
telegram_id, code_verifier = state.split(":", 1) if ":" in state else (state, "")
# exchange_and_store takes (code, code_verifier) — reads identity from HERMES_SESSION_USER_ID
...
```python
# has_token() and get_valid_access_token() take zero arguments
if has_token():
    token = get_valid_access_token()  # auto-refreshes if needed
...
os.environ['GWS_VAULT_SOCKET'] = '/run/gws-vault/vault.sock'
# Set HERMES_SESSION_USER_ID if kelsa_list_tools fails with "No session user context"
# os.environ['HERMES_SESSION_USER_ID'] = '7449813913'
from tools.kelsa_auth import get_valid_access_token

token = get_valid_access_token()  # zero args — reads identity from session env
...
async def query_kelsa():
    ...
    token = get_valid_access_token()  # zero args
...
|----------|-----------|---------|
| `get_auth_url` | `() -> str` | Authorization URL to send user (reads identity from session env) |
| `parse_callback_paste` | `(pasted: str) -> (code: str, state: str)` | Extracts code and state from redirect URL |
| `exchange_and_store` | `(code: str, code_verifier: str)` | Exchanges auth code for tokens, stores in vault (reads identity from session env) |
| `get_valid_access_token` | `() -> str` | Gets current access token, auto-refreshes if expired (reads identity from session env) |
| `has_token` | `() -> bool` | Checks if a token exists in vault (reads identity from session env) |
...
- **`has_token()` can return cross-user false positives** — when the canonical UID (e.g. `ndr-[REDACTED-TID]`) isn't found directly in the vault, the fallback resolves to the raw ID (`[REDACTED-TID]`) and may find a token belonging to a different user entirely. If the user insists they have no Kelsa token despite `has_token()` returning True, trust the user and generate a fresh auth URL.
