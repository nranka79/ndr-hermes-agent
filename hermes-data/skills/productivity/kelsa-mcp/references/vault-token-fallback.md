...
os.environ['HERMES_SESSION_USER_ID'] = '[REDACTED-TID]'  # e.g. Nishant's id; see memory for Bharat's slug (sales1_blr)
...
```
cd /opt/hermes && source /opt/hermes/.venv/bin/activate 2>/dev/null; HERMES_SESSION_USER_ID=[REDACTED-TID] python3 /tmp/kelsa_find_manju.py
```

## API signature changes (verified Jul 2026)

1. **`get_valid_access_token()` takes 0 positional args.** The old call
   `get_valid_access_token("[REDACTED-TID]")` raises
...
1-arg signature `get_valid_access_token('[REDACTED-TID]')` and will fail with
