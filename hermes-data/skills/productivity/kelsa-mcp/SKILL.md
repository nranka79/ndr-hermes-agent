...
| `move_stage` | Enforces prerequisites and automations. **⚠️ Sequential progression only — cannot jump stages.** E.g. Pipeline 10: Cold -> Warm (ID 2) -> PSC (ID 281) -> SSV (ID 6). From Cold, only Warm is allowed. From Warm, only PSC (281), not SSV directly. Attempting a jump returns: "The record cannot jump to X from its current stage." **⚠️ Cannot target retired stages** (Junk, Dead, Lost) — the API omits them from allowed targets entirely. **⚠️ Requires pipeline_id.** See `references/batch-processing-and-movestage.md` for full details and workarounds. |
...
print(has_token("[REDACTED-TID]"))  # their telegram_id
```
...
- Gateway-level tools (`kelsa_list_tools`/`kelsa_call_tool`) return `"Kelsa connection failed: unhandled errors in a TaskGroup (1 sub-exception)"` — especially when the MCP client's stored token has expired
...
2. **MCP server token (managed by the Hermes MCP client)** — set up via `hermes mcp add Kelsa-Read --auth oauth` (background PTY flow). The MCP client stores its credentials internally — never look for or inspect token files on disk; `hermes mcp test Kelsa-Read` reports validity. This is what the gateway's MCP connection uses.
...
| `mcp list` | `mcp test` | Credential state (managed internally by the MCP client — do NOT inspect on disk) | Meaning |
|-----------|-----------|----------------------|---------|
| **No MCP servers configured** | N/A | OAuth completed at some point (e.g. via a side channel) | **MCP server config lost but OAuth was completed at some point.** The server entry was removed from state.db or never persisted. Fix: run `hermes mcp add Kelsa-Read --url "https://kelsa.io/mcp" --auth oauth`. If the existing credentials are still valid, this may skip OAuth entirely and jump straight to the tool-enable prompt (`"Enable all N tools? [Y/n/select]:"`) — answer `"Y"`. |
| **No MCP servers configured** | N/A | No credentials (OAuth never completed) | **OAuth never completed or config lost before any credentials were exchanged.** Run fresh `hermes mcp add Kelsa-Read --url "https://kelsa.io/mcp" --auth oauth` via background PTY — full OAuth flow required with user browser authorization. |
| enabled | succeeds | Credentials valid | Connection is live. Start a new conversation. |
| enabled | 401 | Credentials unreadable by the hermes user | Auth broken (permission error). Gateway shows config-valid but auth is broken. See "Token File Permissions" below. |
| enabled | "authorization required" + connection timeout | Token expired | Token expired. The CLI detects the expired token, starts a new OAuth flow (shows "authorization required"), then times out because the server waits for user interaction. Re-auth via `hermes mcp add` as background PTY process (see "OAuth in a Non-Interactive (Headless) Environment" below). `hermes mcp login` fails in headless. |
| enabled | 401 | Token actively rejected | Token actively rejected (not just expired). Same fix as above — re-auth via background PTY. |
| enabled | 401 | No credentials persisted | Either the flow was never completed, or credentials were exchanged via a side channel (Python script, direct HTTP call) that skipped the CLI's persistence layer. In the side-channel case the gateway may have a valid connection (credentials in process memory) but the Hermes CLI can't use them. Run full OAuth re-auth via background PTY `hermes mcp add`. |

**Key distinction — OAuth completed but no server config:** This is the most confusing state because it looks like nothing was done ("No MCP servers configured") but credentials exist. Correlate when OAuth was last completed via `hermes mcp test Kelsa-Read` results or gateway connection logs — useful for correlating with scope changes (e.g. `kelsa_auth.py` scope widened on Jul 20).

If diagnostics show the permission-error outcome (enabled + 401 + unreadable credentials), re-run the `hermes mcp add` auth flow before proceeding to resolution paths — that may be the only issue. The remaining 401 outcomes mean OAuth was configured but never authorized or the token expired — run the re-auth fix.
...
3. **Direct HTTP calls via `mcp` Python package** — For one-off queries when neither above is possible:
   ...
   ```
   Requires the OAuth token — obtain it from the vault via `tools.kelsa_auth.get_valid_access_token()`, or use the MCP manager's provider. Never read token files from disk.
   ...
   **Fix:** Run `hermes mcp add Kelsa-Read --url "https://kelsa.io/mcp" --auth oauth` as a background PTY process (see "OAuth in a Non-Interactive (Headless) Environment" section below) to re-auth and fix credential ownership. Or, if the gateway already has the live connection (token in process memory), start a **new conversation** — the tools become available at session init but cannot be extracted from the running gateway process by a subprocess.
...
4. Fallback: Kelsa web UI — Direct the user to the Kelsa app URL with a filter:
   ...
   **Correlate OAuth completion time** — check when `hermes mcp test Kelsa-Read` last succeeded or grep the gateway log for Kelsa-Read connection events. If `mcp list` shows "No MCP servers configured" but credentials were previously valid, the OAuth was done via a side channel or the config was lost. Reliable fallback: Kelsa web UI
...
**Key assets for troubleshooting (no token-file inspection):**
- Auth state: `hermes mcp test Kelsa-Read` — the CLI reports whether stored credentials are valid. Do NOT inspect or read token files on disk.
...
**Fix:** Run `hermes mcp add Kelsa-Read --url "https://kelsa.io/mcp" --auth oauth` as a background PTY process (see "OAuth in a Non-Interactive (Headless) Environment" below). This re-auths and rewrites the credentials with correct `hermes:hermes` ownership. `hermes mcp login` fails with "non-interactive environment" even with pty=true. `chown` via sudo is unavailable in the container.
...
4. **Verify token stored** in vault:
   ...
   sys.path.insert(0, '/opt/hermes')
   from tools.kelsa_auth import has_token
   print('Token in vault:', has_token('[REDACTED-TID]'))
   "
   ```
...
6. **If CLI insists on re-auth** (shows OAuth URL again despite vault token existing), the CLI's OAuth provider cannot read the vault token — it only reads its own internally-managed credentials. This means the vault token and the MCP client token are independent. You may need to do both: authorize via HTTPS callback (stores in vault) AND submit the paste-back to the CLI process (stores on disk). This is a structural limitation — the two systems don't share state.
...
- **`hermes mcp add` can hang indefinitely at "completing flow" after paste** — After pasting the redirect URL, the process outputs `"Got authorization code from paste — completing flow."` but never finishes (observed >90s without completion). The process doesn't error or show success — it just sits there.
  ...
  **⚠️ Critical — PTY stdin ordering pitfall (Jul 2026):** When the timeout fires, TWO prompts are pending on the same stdin — the paste prompt (`"Or paste the redirect URL..."`) and the save-config prompt (`"Save config anyway... [y/N]:"`). The paste prompt's reader is state-machine-registered FIRST, so `process(action="submit", data="y")` goes to the paste-prompt reader (which rejects it as not containing `code=`) — NOT the save-config prompt. You must submit the paste URL FIRST (satisfies paste-prompt reader), THEN "y" (reaches save-config reader).
  ...
  4. Token exchange should complete cleanly. Verify with `hermes mcp test Kelsa-Read` — the CLI reports whether auth is valid; no token-file inspection needed.
  5. If process hangs at "completing flow" after both inputs, wait 15-30s then run `hermes mcp test Kelsa-Read` — it may have exchanged successfully despite the hang
  6. If auth is still not valid, kill and restart with a fresh `hermes mcp add` — both the old OAuth URL and auth code are one-time-use
  ...
  **Path B (paste was already submitted, now hanging at "completing flow"):**
  ...
  2. **Check whether auth was actually exchanged** even though the process hung: run `hermes mcp test Kelsa-Read`.
  3. If auth is valid, restart the gateway to pick it up: `/opt/hermes/.venv/bin/hermes gateway restart` (with user approval)
  4. If auth is still invalid, start a fresh `hermes mcp add` — the old auth code is one-time-use, so a new OAuth URL is required
  ...
  **Takeaway:** If you killed a hung process, always verify with `hermes mcp test Kelsa-Read` and try a fresh `hermes mcp add` before starting a new OAuth URL flow — especially if the gateway was restarted in the meantime.
...
- **`kelsa_login` may falsely return "Already authorized" due to session context mismatch.** The `_current_telegram_id()` function returns a canonical UID with a user prefix (e.g. `ndr-[REDACTED-TID]`). If a token exists in the vault under the raw Telegram ID (`[REDACTED-TID]`) from a different user (Nishant, not Roshini), `has_token()` may find it via vault fallback logic and the tool returns "Already authorized" — even though the current session user has no Kelsa token of their own. The user's statement overrides the tool response: if they say "the token doesn't exist," trust them and force a fresh auth URL.
...
**⚠️ Terminal user context for Gmail access:** When accessing Gmail from terminal (not execute_code), `HERMES_SESSION_USER_ID` determines which OAuth token loads. The terminal inherits the gateway user (often `ndr-[REDACTED-TID]` = Nishant) — but portal leads land in Bharat's mailbox (`sales1.blr@draas.com`, Telegram ID `[REDACTED-TID]`). Override explicitly:
```bash
HERMES_SESSION_USER_ID=[REDACTED-TID] GWS_VAULT_SOCKET=/run/gws-vault/vault.sock /opt/hermes/.venv/bin/python3 -c "..."
```
**⚠️ But the google-draas token may be stored under psingh-[REDACTED-TID], not sales1.blr-[REDACTED-TID].** If `VaultNoTokenError` occurs, override to `HERMES_SESSION_USER_ID=[REDACTED-TID]` instead. See `references/gws-session-context-bharat.md`.
