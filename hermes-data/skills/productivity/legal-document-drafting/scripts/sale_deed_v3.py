#!/usr/bin/env python3
"""Generate a formatted DRAAS Absolute Sale Deed as .docx.

Usage:
    uv run --with python-docx python3 scripts/sale_deed_v3.py

Output: /tmp/sale_deed_v3.docx (47 KB)

Built with python-docx. Covers: parties, background & recitals (SPA, chain
of title, layout approvals), 10 operative clauses, execution blocks.
"""
...
# C
heading(doc, "C.  LAYOUT DEVELOPMENT AND APPROVALS", size=12, space_b=8, space_a=4)
...
sp(doc, 'PLACE: Bagalur, Krishnagiri District, Tamil Nadu     |     DATE: _____ May 2026',
   bold=True, space_b=6, space_a=2)

sp(doc, 'DRAFTED BY: [Advocate / Document Writer Name] | [Enrollment No.] | [Full Address] | [Contact Number]',
   bold_phrases=["DRAFTED BY:"], space_b=2, space_a=4)
...
annexures = [
    ("Annexure A", "Encumbrance Certificate dated 21.05.2026 (CLEAR) -- Search Period: 01.01.1975 to 20.05.2026, Sub Registrar Bagalur"),
    ("Annexure B", "Layout Approval -- Hosur New Town Development Authority, Approval Nos.: SWP Vz: 90/2025 and 38/2025"),
    ("Annexure C", "Gift Deed Doc No.1634/2025 -- Roads and Common Areas gifted to TANGEDCO, dated 24.02.2025, Sub Registrar Bagalur"),
    ("Annexure D", "Gift Deed Doc No.1632/2025 -- Roads and Common Areas gifted to Sevaganapalli Panchayat, dated 24.02.2025, Sub Registrar Bagalur"),
    ("Annexure E", "Copy of Registered Sale Deed Doc No.20527/2024 -- Source Title Deed of Vendor, dated 24.10.2024, Sub Registrar Hosur"),
]

for annex, desc in annexures:
    p = doc.add_paragraph()
    pf = p.paragraph_format
    pf.left_indent = Inches(0.5)
...
# Save
import os
output_path = '/tmp/sale_deed_v3.docx'
doc.save(output_path)
print(f"Saved. Paragraphs: {len(doc.paragraphs)}")
