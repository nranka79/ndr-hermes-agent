...
_HEADER_SYNONYMS = {
    ...
    "psf": ["currentpricepersqft", "persqft", "persqftrate", "psf",
            "pricepersqft", "per sq ft", "pricepsf", "pricesqft",
            "pricepersqft"],
    ...
    "source_url": ["sourceurl", "source", "listingurl", "url"],
    "confidence": ["confidence"],
    "date": ["date", "listingdate", "updated"],
    "portal": ["portal", "sourceportal", "website"],
    "area": ["area", "areaindicator", "sqftarea", "areasqft"],
    "notes": ["notes", "remarks"],
}
...
def append_rows(sheet_id, tab, rows, start="A1",
    ...
    if rows and isinstance(rows[0], dict):
        ...
        # NOTE (Aug-2026): keep None-mapped columns (e.g. a '#' index column)
        # as empty cells. Dropping them shifted every value one column LEFT
        # and corrupted appended rows (project landed in col A, type in B,
        # lat in the Units column, etc.).
        values = []
        for rec in rows:
            values.append([rec.get(f, "") if f is not None else ""
                           for f in mapping])
    body = {"values": values}
...
def update_drive_file(file_id, content_bytes, service_name="google-draas",
    ...
    NOTE (Aug-2026): googleapiclient's media_body rejects raw bytes
    (TypeError: media_filename must be str or MediaUpload) — wrap with
    MediaIoBaseUpload.
    """
    import io
