...
from sheet_io import (  # noqa: E402
    ascii_fold, coord_bucket, key_name, parse_psf, parse_coord, read_records,
)
...
def label_for(rec, with_price=True):
    ...
    if rec.get("total"):
        # total-only fallback (NDR pattern: `NAG Green Park | Rs 32 - 50 L`)
        return f"{name} | {rec['total']}"
    return name
...
def description_for(rec, sources):
    ...
    if sources:
        for i, s in enumerate(sources, 1):
            ...
            if s.get("psf") or s.get("price"):
                parts.append(f"Rs {s.get('psf') or s.get('price')}")
            if s.get("total"):
                parts.append(f"({s['total']})")
            if s.get("date"):
                parts.append(f"[{s['date']}]")
            if s.get("source_url") or s.get("url"):
                parts.append(s.get("source_url") or s.get("url"))
...
def build_kml(competitors, pois, subject=None, title="R&D Map",
    ...
    for rec in competitors:
        rec["lat"] = parse_coord(rec.get("lat"))
        rec["lon"] = parse_coord(rec.get("lon"))
