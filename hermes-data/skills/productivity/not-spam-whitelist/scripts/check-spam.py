...
def read_whitelist(sheets):
    ...
    for i, row in enumerate(rows[1:], 2):
        ...
        # ── Infer rule_type when column G is empty ──────────────
        # Many whitelist rows have an empty Rule Type column (G).
        # Infer from the data format:
        #   '@domain'       → domain_from
        #   'user@domain'   → exact_from
        #   bare domain + subject keywords → combined
        #   bare domain, no keywords → domain_from
        #   fallback → exact_from
        if not rules[-1]["rule_type"]:
            fv = rules[-1]["from_value"]
            if fv.startswith("@"):
                rules[-1]["rule_type"] = "domain_from"
            elif "@" in fv:
                rules[-1]["rule_type"] = "exact_from"
            elif rules[-1]["subject_kw"]:
                rules[-1]["rule_type"] = "combined"
            elif "." in fv:
                rules[-1]["rule_type"] = "domain_from"
            else:
                rules[-1]["rule_type"] = "exact_from"
