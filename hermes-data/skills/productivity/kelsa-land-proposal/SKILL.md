...
description: >-
  ...
  Invoice Processing (ID 516), Vendor Shortlisting (ID 531), Companies Master
  (ID 4475), PO-WO Issuing (ID 537). Covers: pipeline discovery, field mapping,
  ...
  "create a vendor in Kelsa", "update this lead in Kelsa".
metadata:
  hermes:
    tags:
      - real-estate
...
4. **Disambiguate similar parcels with the map pin:** resolve both the shared maps.app.goo.gl link and the candidate lead's `cf_location_google_maplink` via `curl -s -L <url> -w "%{url_effective}"` and extract `@lat,lng` / `q=lat,lng`. Identical-ish coordinates = same property; different coordinates = different parcel even if names look similar (e.g. Thylagere 10A pin 13.3216,77.6789 vs Nandi-backside pin 13.3297,77.6007 are different properties).
...
result = kelsa_call_tool(
    ...
    arguments={
        ...
        "content_type": "application/pdf"  # or video/mp4, image/png, etc.
    }
)
# Returns: upload_url (S3 endpoint), fields (form fields), file_url
```
...
These are observed from existing records — verify against live pipeline before use:
...
- **Product Type**: Villa, Row House, Apartment - High Rise (>G+8), Plots, Farm Plots, **Commercial**, etc.
...
5. **Drop down values are not returned by `get_pipeline`.** The API response says "dropdown (N options)" but doesn't list them. To discover valid options, examine an existing lead via `get_lead`.
...
25. **OAuth required for Kelsa MCP — headless workaround.** The Kelsa-Read MCP server requires OAuth 2.1 PKCE. In a headless environment, the local callback server at `http://127.0.0.1:<port>/callback` is unreachable from the user's browser. Three approaches:
    ...
    - **Option B (DCR client — transcribe.ahfl.in proxy):** A pre-registered DCR client was attempted at `/data/hermes/mcp-tokens/kelsa-read-dcr-client-v2.json` with `redirect_uri: https://transcribe.ahfl.in/kelsa/auth/callback` (confirmed HTTP 200 as of 2026-07-24). **⚠️ VERIFIED EMPTY (0 bytes) as of 2026-07-29** — this file was created but never populated with a valid DCR client configuration. To use this approach, the file must first be populated with a valid `client_id`, `client_secret`, `redirect_uri`, and `token_endpoint` before the proxy can handle OAuth callbacks.
    - **Option C (fallback):** If OAuth cannot be completed, use Kelsa's web UI directly. Ask the user to create leads manually or paste lead URLs. Do not block on MCP availability.
