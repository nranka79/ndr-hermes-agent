...
**Identity-resolution pitfall (ALL session types, not just cron):** This is NOT limited to cron/auto sessions. The direct `kelsa_list_tool`/`kelsa_call_tool` tools resolve identity from the session env and can fail with 'No session user context' in ANY session type — including interactive Telegram — when the resolved vault user doesn't hold a Kelsa token. Example: Nishant's `ndr` Telegram session (`.hermes` config pointing to ndr) triggers the NDR token but the direct tools fail, while the terminal fallback with `HERMES_SESSION_USER_ID=ndr-7449813913` works fine. Bharat's `sales1.blr` typically works through the direct tools because his vault identity resolves properly.
...
**Mentions in notes:** put the token straight into `add_note` text. Mentioning a user also adds them as a follower of the lead and sends them a notification. This is the lightweight way to route a record to a person (e.g. a reimbursement note tagging `@[Eshwari](702) @[Sarthak Sharma](3511)` to ask accounts to make the entry) — no separate add-follower call needed.

**Tool-arg pitfalls (verified 2026-08-20):** `add_note` and `get_lead` REQUIRE `pipeline_id` in their arguments, unlike many other tools that accept a bare record id. Omitting it returns `Missing required arguments: pipeline_id`. Always pass `{"pipeline_id": <id>, "lead_id": <id>}` for get_lead and `{"pipeline_id": <id>, "lead_id": <id>, "text": ...}` for add_note.
...
2. Upload the file bytes to S3 via **multipart/form-data POST**. Every returned field plus a `file` field (the raw bytes) must be included. The `file` field MUST come last. The presigned POST enforces 25 MB max (`content-length-range [0, 26214400]`) — oversized PDFs should be compressed with Ghostscript before uploading. Use `curl` or Python `requests`:
...
**DRA Land Proposal Pipeline 519 session reference:** See `references/lalbagh-annipura-land-proposal-2026-08-26.md` for the `cf_name` vs `name` parameter pitfall, S3 25 MB upload limit with Ghostscript fix, Drive native-PDF download pattern, and title-detail extraction from scanned HC orders.
...
| "Invoice" / "invoice pipeline" / "approve this invoice" | DRA Invoice Processing | 516 |
...
**Document classification guide:** When the user submits files (PDFs, images) with a land proposal, see `references/document-classification-for-land-proposals.md` for how to examine each file's content and map it to the correct attachment field (Land Sketch, Google MapLink, Revenue Maps, Land Legal Set, etc.).
⚠️ **`cf_expected_jv_ratio_for_developer` MCP pitfall:** This field is auto-calculated (100 - Land Owner ratio). MCP `create_lead` / `update_lead` cannot override it — updates silently fail and MCP returns stale values. Set `cf_expected_jv_ratio` (Land Owner) correctly; the Developer side should follow from the engine. If the display is wrong, the user needs to adjust via the Kelsa UI.

**DRA Invoice Processing (ID: 516) — invoice approval workflow:** Vendor invoices (Vardhan Ventures, Lakshmi Steel Traders, etc.) come in as a single multi-page scanned PDF whose later pages are the supplier slip (proof of delivery) and the mill test certificate (proof of quality). The approval flow is: attach proof-of-quality + proof-of-completion PDFs (extracted as separate files from the one uploaded invoice PDF), verify quantity/spec against the invoice, add a note, then complete the "Issuer of PO-WO to verify… approve invoice for payment" task → record advances to "Approved by the Issuer of PO/WO". See `references/dra-invoice-processing-pipeline.md` for the full recipe (page-extraction, upload, field IDs, task flow, note format).

**Pipeline 10 (DRA Sales Leads) — phone search: WORKS via IDEN identifier line (updated Aug 2026).** The phone number is stored via a **master field** (`cf_contact_phone` → DRA Sales Contacts pipeline 3429), but `search_leads(pipeline_id=10, query="XXXXXXXXXX")` with the **bare 10-digit phone** DOES match existing leads through the auto-generated identifier line (`[#54865031] Tamilselvan-["+917094183714"]`). Verified 2026-08-25: searching `9449599199` returned the June Suresh lead; searching phones of leads that don't exist returned 0. So phone search on Pipeline 10 is a valid quick dedup check (contrary to older notes claiming it always returns 0). For authoritative confirmation (phone is a field on the linked contact), ALSO search the **contacts pipeline (3429)** — 0 hits in both = genuinely new lead. The contacts pipeline search is the ground truth before creating duplicates.
...
- **As of Jul 2026:** Pipeline 10 was reported as "0 records accessible" under some tokens — this is a **permissions/token artifact, not an empty pipeline**. The pipeline is live with hundreds of records (e.g. 22 leads in SSV alone). Always verify with `get_stats` and try the ndr token (`HERMES_SESSION_USER_ID=[REDACTED-TID]`) before assuming records don't exist.
...
- **Pipeline 519 `cf_name` trap:** `create_lead`'s `name` parameter sets the record title but does NOT satisfy the `cf_name` (Proposal Brief) prerequisite for the Proposed stage. Both must be set: `name="..."` AND `field_values={"cf_name": "..."}`. Draft fails with "Required fields not present: Proposal Brief" if omitted.
...
- **⚠️ Export xlsx may have VLOOKUP formulas pointing at external workbook (verified 2026-08-28):** Openpyxl reads only the formula text (`=IFERROR(VLOOKUP(C2,'[1]consolidated File'!$C:$O,9,0),...)` — not the evaluated result. The cached values are in a sibling `(Data).csv` in the same document_cache folder (e.g. `doc_<id>_Ranka Udaya... (Data).csv`). Read with `encoding='latin-1'`. Always check for the companion CSV before concluding the sheet has no call data.
- **Pipeline summary sheet creation:** After bulk-updating leads, the user often asks for a summary worksheet. See `references/creating-pipeline-summary-sheet.md` — covers get_stats → search_leads by stage → multi-tab Google Sheet with formatting, plus the apostrophe-in-sheet-name batchUpdate workaround.
- **⚠️ Long batch runs drop MCP connection after ~100 calls (verified 2026-08-28, 195-lead Ranka Udaya run):** A single `process_batch_json.py` run on ~110 leads caused `MCP server 'kelsa-read-pilot' connection lost` and `McpError: Internal error` on `move_stage` mid-batch. **Proven split strategy:** (1) process the notes-only actions (not_answered, note_only, junk — no stage move) in one batch — 56 leads ran clean. (2) handle stage-move leads (warm, ssv) via direct `kelsa_call_tool` calls — search by phone (IDEN line matches), add_note, update_lead cf_requirements, then move_stage. Keep each batch ≤ ~60 leads. Build the split JSON files with a small Python script and resume from the last index that succeeded.
- **⚠️ `batch_import_leads.py` reports false "Failed" on async lead creation (verified 2026-08-25):** When `create_lead` returns "Record queued for processing", the script's 2s `get_draft_status`/phone-search fallback often misses the new lead ID and marks it **Failed** — BUT the lead is actually created successfully moments later. After a run reports all-Failed, DON'T re-run (that would duplicate the contacts/leads). Instead verify each phone/name exists in Pipeline 10 via `search_leads` and report the actual record IDs. Leads appear with the correct phone in the identifier line (e.g. `[#54929265] Praveen-["+919791238005"]`). Also NOTE: the script's `get_valid_access_token('[REDACTED-TID]')` call was patched to `get_valid_access_token()` (no args — identity comes from `HERMES_SESSION_USER_ID`); run it with `HERMES_SESSION_USER_ID=sales1_blr GWS_VAULT_SOCKET=/run/gws-vault/vault.sock`.  
**⚠️ cf_contact pitfall:** When creating a contact in Pipeline 3429, ALL fields (name + phone + email) MUST go inside the `cf_contact` nested dict. See `references/housing-com-batch-import.md` for the correct pattern and `references/single-lead-import-pipeline10.md` (pitfall section) for detail on the blank-name bug.  
**Housing.com portal imports** (distinct source from Meta): Channel=Portals, Source=Housing.com. See `references/housing-com-batch-import.md` for the full workflow, pre-processing (phone format, name bracket removal), and the direct MCP JSON-RPC pattern used for batch operations.
...
- **Pipeline 10 phone search:** Bare 10-digit phone search DOES match leads via the IDEN identifier line (updated Aug 2026 — see the "phone search: WORKS via IDEN" section above). For authoritative dedup, also check contacts pipeline 3429. The old `references/pipeline10-phone-search-limitation.md` is STALE on the "always 0 results" claim.
- **Calling campaign result updates (new Aug 2026, updated 2026-08-28):** See `references/calling-campaign-update.md` for the full decision-tree workflow. Covers: reading the export file (⚠️ xlsx cells may contain VLOOKUP formulas pointing at external `[1]consolidated File` — openpyxl reads only the formula, not the result; the evaluated values live in a sibling `(Data).csv` in the same document_cache, read with `encoding='latin-1'`), matching by phone, and applying the outcome-based stage change. Covers the "only first N leads" constraint, Teams/SharePoint download, and sequential stage-move gotchas.
- **Batch processing script (JSON input, FIXED 2026-08-28):** `scripts/process_batch_json.py` — a re-runnable script that takes a JSON array of lead objects (phone, action, remarks), searches each by phone in Pipeline 10, adds the appropriate note, and moves the stage (Warm/PSC). Handles all edge cases: leads already in target stage, retired stages (Junk), and the cf_requirements prerequisite for Warm. Invoke with `HERMES_SESSION_USER_ID=<session_telegram_id> python3 skills/productivity/kelsa-write/scripts/process_batch_json.py <file.json>` (the script lives under the skill dir, NOT /data/hermes/scripts/). **Fixed 2026-08-28:** The old `_run_on_mcp_loop(_worker())` call crashed with `_worker() missing 1 required positional argument: 'session'` — the correct form is `_run_on_mcp_loop(lambda: _connect_and_run(token, _worker), timeout=600)`. The script file has been patched.
...
- **Batch calling campaign update (JSON input):** `templates/process_calling_batch.py` — reusable template to process a JSON array of calling campaign results. Searches by phone in Pipeline 10, adds a note, and optionally moves the stage (Warm/PSC). Handles all three known move-stage no-ops: "already in stage", "cannot jump backward", and "retired stage not reachable". See `references/calling-campaign-update.md` for the decision tree and pitfalls.
