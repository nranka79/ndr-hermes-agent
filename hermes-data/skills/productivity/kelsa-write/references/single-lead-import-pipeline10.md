...
- Working Kelsa token. In cron/auto sessions use:
  `HERMES_SESSION_USER_ID=[REDACTED-TID] GWS_VAULT_SOCKET=/run/gws-vault/vault.sock`
  (ndr-[REDACTED-TID] holds `mcp-kelsa-read`). See SKILL.md identity pitfall.
...
2. **Create contact (3429)** — `cf_contact` is a compound field. You MUST
   pass all contact info (name + phone + email) **inside** the nested dict:
   ```python
   field_values = {
       "cf_contact": {
           "name": lead_name,
   ...
   ```
   ⚠️ **Pitfall (verified 2026-08-22):** Passing `name` as a top-level param +
   separate `cf_contact_email`/`cf_contact_phone` fields WITHOUT a proper
   `cf_contact` dict creates the contact with a **blank name field**. The name
   in the display becomes `-[...+919XXXXXXXXX]` instead of
   `Name-[...+919XXXXXXXXX]`. When this lead is linked to Pipeline 10, the
   name stays blank on both the contact and the lead. The only reliable
   pattern is ALL fields inside `cf_contact: {name, phone, email}`.
