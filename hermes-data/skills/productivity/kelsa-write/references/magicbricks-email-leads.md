...
- **Empty search is NOT JSON:** `gws_skill_bridge.call("gmail_search", ...)` returns the literal string `"No messages found.\n"` (not a JSON array) when a query has zero hits. `json.loads(result)` then raises `JSONDecodeError`. Guard before parsing: `if isinstance(res, str) and res.strip().startswith('No messages'): return []`.
