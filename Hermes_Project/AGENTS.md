...
> **Infrastructure context:** For VPS / Docker / Telegram / N8N / GWS / Google Sheets work, read `hermes-data/connections/hetzner.md` first — it has the SSH target, container layout, env vars, N8N workflow IDs, users, and useful commands. The same briefing is also inlined into the global `C:/Users/ruhaan/AGENTS.md` so it loads into every session.
>
> **Sync policy:** files under `hermes-data/` that mirror VPS state (e.g. `users.json`) come from the Hetzner VPS, not from this checkout. Before reading or editing any of them, run `pwsh scripts/sync-from-vps.ps1` to pull the latest version. The VPS is always the source of truth — the local copy is a one-way mirror.
