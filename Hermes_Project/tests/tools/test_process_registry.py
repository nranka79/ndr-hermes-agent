...
class TestSpawnEnvSanitization:
    ...
    def test_spawn_local_injects_contextvar_over_stale_os_environ(
        self, registry, monkeypatch
    ):
        """Background subprocess must see the *current* request's user
        identity (from the session contextvar), NOT a stale
        ``os.environ["HERMES_SESSION_USER_ID"]`` left over from process
        startup or a previous request.

        Regression for the misidentification bug: terminal(background=True)
        was routing the per-user OAuth token to the wrong account because
        ``os.environ`` had stale user identity from prior requests.
        """
        ...
        # Simulate the bug scenario: os.environ has stale identity
        # (e.g. "sales1.blr@draas.com" from a prior request or process
        # startup), while the *current* gateway request's session
        # contextvar is the correct user (e.g. "ndr@draas.com").
        ...
        env = captured["env"]
        # The contextvar (current request) must win over os.environ
        # (stale), for every session var the gateway binds.
        assert env["HERMES_SESSION_USER_ID"] == "ndr@draas.com", (
            f"stale os.environ leaked: got {env['HERMES_SESSION_USER_ID']!r}, "
            f"expected 'ndr@draas.com' from the contextvar"
        )
        assert env["HERMES_SESSION_USER_NAME"] == "Nishant Ranka"
        assert env["HERMES_SESSION_KEY"] == "test-session-key"

    def test_spawn_local_writes_empty_string_when_contextvar_explicitly_cleared(
        self, registry, monkeypatch
    ):
        """When the gateway explicitly cleared a session var (e.g. an
        unauthenticated request, or ``clear_session_vars`` after
        ``set_session_vars``), the subprocess env must contain the empty
        string — NOT whatever value was in os.environ.  This prevents
        a previous request's identity from contaminating a later
        ...
        # 1) bind a user, then 2) clear the contextvar.  The cleared
        # state must still override the os.environ fallback.
        tokens = set_session_vars(user_id="ndr@draas.com")
        clear_session_vars(tokens)
        try:
            with patch.dict(
                os.environ,
                {
                    "PATH": "/usr/bin:/bin",
                    "HERMES_SESSION_USER_ID": "stale-leak",
                },
                clear=True,
            ), \
                patch("tools.process_registry._find_shell", return_value="/bin/bash"), \
            ...
            # Be polite — make sure no contextvar leaks into the next test.
            clear_session_vars(tokens)

        env = captured["env"]
        # After clear_session_vars, the contextvar holds "" (not _UNSET),
        # so inject_session_context_into_env writes "" to env.  This
        # guarantees the os.environ "stale-leak" value is overridden.
        assert env["HERMES_SESSION_USER_ID"] == "", (
            f"explicitly-cleared contextvar should win over os.environ; "
            f"got {env['HERMES_SESSION_USER_ID']!r}"
        )

    def test_spawn_local_does_not_inject_session_vars_when_contextvar_unset(
        self, registry, monkeypatch
    ):
        """CLI / cron / test paths never call ``set_session_vars`` —
        the contextvars remain at their default ``_UNSET`` sentinel.
        In that case, the helper must NOT clobber the caller's env
        (which is built from os.environ).  This preserves the
        CLI/cron fallback path.
        """
        from gateway.session_context import _SESSION_USER_ID, _UNSET
        ...
        # Force the contextvar back to the _UNSET sentinel.  Other tests
        # in this file may have left it at "" (clear_session_vars sets
        # to "" by design, not _UNSET, so the next test sees "explicitly
        # cleared" instead of "never set").  We want the latter here.
        ...
        env = captured["env"]
        # Caller's os.environ value preserved — no clobber.
        assert env["HERMES_SESSION_USER_ID"] == "cli-fallback-user"
