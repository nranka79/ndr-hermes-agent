...
-- Open WebUI / API-server chat_id → parent_session_id mapping.
-- Lets the api_server give the same Open WebUI chat a stable hermes
-- session across requests, so delegate_task subagents get a parent
-- that future requests can find and poll for results.
...
CREATE INDEX IF NOT EXISTS idx_chat_session_map_parent
    ON chat_session_map(parent_session_id);
"""
...
class SessionDB:
    ...
    # ── Open WebUI / api_server chat_id mapping ──────────────────────
    #
    # The api_server is stateless: every POST /v1/chat/completions currently
    # generates a fresh UUID for session_id, so a subagent dispatched in
    # request N is parented to a hermes session that no future request will
    # ever reuse. This mapping gives the api_server a stable parent_session_id
    # for each Open WebUI chat_id (sent via the X-OpenWebUI-Chat-Id header),
    # so the rest of the agent loop — subagent dispatch, async-delegation
    # results, compression, /resume — can all find the session.
    ...
    def get_or_create_chat_session(
        ...
        """Return the persistent parent_session_id for *chat_id*.

        On first call creates a new session row and a chat_session_map row.
        On subsequent calls updates ``last_seen_at`` and returns the same
        parent_session_id — this is what makes subagent polling work
        across multiple requests from the same Open WebUI chat.
        """
        chat_id = str(chat_id)
        with self._lock:
            row = self._conn.execute(
            ...
            if row:
                ...
                self._conn.execute(
                    "UPDATE chat_session_map SET last_seen_at = ? WHERE chat_id = ?",
                    (time.time(), chat_id),
                )
                self._conn.commit()
                return sid

            new_sid = f"owui-{uuid.uuid4().hex[:12]}"
            self._conn.execute(
                "INSERT INTO sessions (id, source, user_id, started_at) VALUES (?, ?, ?, ?)",
                (new_sid, source, user_id, time.time()),
            )
            self._conn.execute(
                "INSERT INTO chat_session_map (chat_id, parent_session_id, last_seen_at) "
                "VALUES (?, ?, ?)",
                (chat_id, new_sid, time.time()),
            )
            self._conn.commit()
            return new_sid

    def get_completed_subagent_results(
        self,
        parent_session_id: str,
        since: float = 0.0,
    ) -> List[Dict[str, Any]]:
        """Return delegate-children of *parent_session_id* that finished after *since*.

        Used by the api_server to surface results back to the parent model on
        the next request — the core of the subagent-polling fix. Returns
        list of dicts (oldest first) with the child session's id, ended_at,
        and a few summary fields.

        Only rows with ``source='subagent'`` are returned so branch children
        and compression continuations (which share the parent_session_id
        link) don't get spuriously injected into the parent context.
        """
        with self._lock:
            rows = self._conn.execute(
                """
                SELECT id, ended_at, end_reason, output_tokens, message_count,
                ...
                """,
                (parent_session_id, float(since)),
            ).fetchall()
        return [dict(r) for r in rows]

    def get_subagent_final_message(self, child_session_id: str) -> Optional[str]:
        """Return the last non-empty assistant message from a child session.

        This is the content injected into the parent's context as a
        ``tool`` role message. Empty / tool-only runs return None so the
    ...
    def get_chat_session_last_injected(self, parent_session_id: str) -> float:
        """Return the last_injected_at marker for a chat-mapped parent session.

        0.0 means never injected. Used to avoid re-injecting the same child
        result on every request.
        """
        with self._lock:
            row = self._conn.execute(
                "SELECT last_injected_at FROM chat_session_map WHERE parent_session_id = ?",
                (parent_session_id,),
            ).fetchone()
        if not row:
            return 0.0
        return float(row["last_injected_at"] or 0.0)

    def mark_subagent_results_injected(
        self, parent_session_id: str, last_injected_at: float
    ) -> None:
        """Advance the last_injected_at marker for the chat-mapped parent.

        Idempotent: caller can pass the same timestamp twice. Only updates
        when the new value is greater (older calls are no-ops).
        """
        with self._lock:
            self._conn.execute(
                """
                UPDATE chat_session_map
                SET last_injected_at = MAX(COALESCE(last_injected_at, 0), ?)
                WHERE parent_session_id = ?
                """,
                (float(last_injected_at), parent_session_id),
            )
            self._conn.commit()
