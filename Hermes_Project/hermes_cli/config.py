...
def check_for_user_level_secrets(hermes_data_dir=None):
    """Scan per-user directories for stray credential files.

    Returns the list of suspicious paths found. Prints a loud warning to
    stderr if any are present. Does NOT auto-delete; that is a manual
    admin step.

    Called from save_env_value() so that every `hermes config set` and
    every `hermes setup` wizard save triggers the scan.

    Why: In June 2026 a stale ``.openrouter_key`` was found at
    world-readable mode 0644 in a user dir. It was not read by any Hermes
    code, but the LLM could have discovered it via ``execute_code`` and
    exfiltrated it. This check prevents the pattern from re-emerging.

    Whitelisted (skipped) subdirectories inside a user dir:
      - .gbrain/, .gbrain-writable/  (gbrain storage, not credentials)
      - node_modules/, .npm/, .cache/, .bun/  (toolchains, not credentials)
      - .locks/  (lock files)
    """
    if hermes_data_dir is None:
        # Try multiple env vars in order of preference. The container sets
        # HERMES_HOME=/data/hermes and its users dir is $HERMES_HOME/users.
        # The host (non-container) install uses /opt/hermes/hermes-data.
        for var in ("HERMES_DATA_DIR", "HERMES_HOME", "HERMES_DATA_PATH"):
            val = os.environ.get(var, "").strip()
            if val:
                hermes_data_dir = Path(val)
                break
        else:
            hermes_data_dir = Path("/opt/hermes/hermes-data")
    users_dir = hermes_data_dir / "users"
    if not users_dir.is_dir():
        return []

    _suspicious_patterns = (
        ".*key", ".*token*", ".*secret*", ".*credential*",
        "*_token.json", "*_key.json", "*credentials*.json",
        "service_account*.json",
    )
    _skip_subdirs = (".gbrain", ".gbrain-writable", "node_modules", ".npm", ".cache", ".bun", ".locks")
        ...
        for pattern in _suspicious_patterns:
            for f in user_dir.glob(pattern):
                ...
                try:
                    ...
                    rel = f
                if any(part in _skip_subdirs for part in rel.parts[:-1]):
                    continue
                suspicious.append(f)
    ...
    if suspicious:
        ...
        print("  WARNING: per-user credential files detected", file=sys.stderr)
        print("=" * 78, file=sys.stderr)
        print(file=sys.stderr)
        print("Hermes stores ALL API keys in /opt/hermes/.env. The following", file=sys.stderr)
        print("files in user directories look like credentials and should be", file=sys.stderr)
        print("deleted. Their content must be moved to /opt/hermes/.env first.", file=sys.stderr)
        print(file=sys.stderr)
        for f in suspicious:
            try:
                st = f.stat()
                mode_str = stat.filemode(st.st_mode)
                print(f"  {mode_str}  {f}  ({st.st_size} bytes)", file=sys.stderr)
            except OSError:
                print(f"  ???  {f}", file=sys.stderr)
        print(file=sys.stderr)
        print("To remove:", file=sys.stderr)
        print(f"  rm <file>", file=sys.stderr)
        print(file=sys.stderr)
        print("If a key was world-readable, rotate it at the provider's console", file=sys.stderr)
        print("BEFORE deleting the file, then add the new key to /opt/hermes/.env", file=sys.stderr)
        print("via 'hermes config set <KEY> <new_value>'.", file=sys.stderr)
        print("=" * 78, file=sys.stderr)
        print(file=sys.stderr)
    return suspicious
...
def save_env_value(key: str, value: str):
    ...
    # Scan per-user dirs for stray credential files. Loud warning only on hit;
    # silent in the clean case. Cheap (one glob per user dir).
    try:
        check_for_user_level_secrets()
    except Exception as _exc:
        logger.debug("check_for_user_level_secrets failed: %s", _exc)
