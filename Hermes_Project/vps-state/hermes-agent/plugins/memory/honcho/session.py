...
class HonchoSessionManager:
    ...
    def get_or_create(self, key: str) -> HonchoSession:
        ...
        _ai_peer_base = self._config.ai_peer if self._config else "hermes-assistant"
        _pin_peer = bool(getattr(self._config, "pin_peer_name", False)) if self._config else False

        # Multi-user isolation: use a per-user AI peer when running in gateway
        # mode with distinct runtime identities.  A shared AI peer accumulates
        # observations from ALL sessions it participates in across every user,
        # so asking it about user B from user A's context leaks cross-user
        # history.  Per-user AI peers are isolated: hermes-assistant-<uid>
        # only ever exists in that user's sessions, preventing cross-user bleed.
        # Single-user deployments (pin_peer_name=True) keep one shared AI peer.
        if self._runtime_user_peer_name and not _pin_peer:
            assistant_peer_id = self._sanitize_id(f"{_ai_peer_base}-{user_peer_id}")
        else:
            assistant_peer_id = self._sanitize_id(_ai_peer_base)
    ...
    def _resolve_peer_id(self, session: HonchoSession, peer: str | None) -> str:
        """Resolve a peer alias to this session's user or assistant peer ID.

        Only "user" and "ai" aliases are accepted.  Any other string is
        rejected and falls back to the session user peer — this prevents the
        LLM (or a malicious prompt) from targeting another user's peer ID
        and reading cross-user memory via the honcho_* tool arguments.
        """
        ...
        # Reject arbitrary peer IDs — only session-owned aliases are valid.
        logger.warning(
            "Rejected out-of-session peer ID '%s' (session user: %s); defaulting to session user",
            normalized, session.user_peer_id,
        )
        return session.user_peer_id
