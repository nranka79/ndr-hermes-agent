...
def _resolve_telegram_user_id(telegram_id: str) -> str:
    """Map a Telegram numeric user ID to the internal user ID via the identity registry.

    For all currently registered users the internal user ID equals the Telegram ID,
    so this is a no-op today. It becomes meaningful when a user is looked up by an
    alias (e.g. a future WhatsApp or Slack ingress) and the same internal user ID
    must be returned regardless of which channel was used.

    Falls back to the raw telegram_id if no registry entry is found.
    """
    try:
        from tools._identity_resolver import resolve_or_fallback
        return resolve_or_fallback("telegram", telegram_id)
    except Exception:
        return telegram_id
...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def _build_message_event(
        ...
        source = self.build_source(
            ...
            chat_type=chat_type,
            user_id=(
                _resolve_telegram_user_id(str(user.id))
                if user
