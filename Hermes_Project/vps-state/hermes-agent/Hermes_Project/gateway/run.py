...
class GatewayRunner:
    ...
    async def _handle_message(self, event: MessageEvent) -> Optional[str]:
        ...
        # -----------------------------------------------------------------
        # Images: inject a plain context note only.
        # The agent must NOT describe, analyze, or read any image unless the
        # user explicitly asks. No auto-vision call is made here.
        # -----------------------------------------------------------------
        message_text = event.text or ""
        if event.media_urls:
            import os as _os_img, re as _re_img
            for i, path in enumerate(event.media_urls):
                ...
                if is_image:
                    basename = _os_img.path.basename(path)
                    parts = basename.split("_", 2)
                    display_name = parts[2] if len(parts) >= 3 else basename
                    display_name = _re_img.sub(r'[^\w.\- ]', '_', display_name)
                    context_note = (
                        f"[The user sent an image: '{display_name}' ({mtype or 'image'}). "
                        f"The file is saved at: {path}. "
                        f"DO NOT describe, analyze, or read this image unless the user explicitly asks. "
                        f"Use the user's message to determine what to do with it.]"
                    )
        ...
        if event.media_urls and event.message_type == MessageType.DOCUMENT:
            for i, path in enumerate(event.media_urls):
                ...
                if mtype.startswith("text/"):
                    ...
                    context_note = (
                        f"[The user sent a document: '{display_name}' ({mtype}). "
                        f"The file is saved at: {path}. "
                        f"DO NOT read, open, parse, or extract content from this file unless the user explicitly asks. "
                        f"Use the user's message to understand what they want done with it.]"
                    )
                message_text = f"{context_note}\n\n{message_text}"
    ...
    def _set_session_env(self, context: SessionContext) -> None:
        ...
        # Expose user_id and user_name so tools can scope per-user credentials without
        # LLM involvement. n8n_tool reads these to inject telegram_id into every GWS
        # webhook call, and to populate the user registry on first auth.
        if context.source.user_id:
            os.environ["HERMES_SESSION_USER_ID"] = str(context.source.user_id)
        elif "HERMES_SESSION_USER_ID" in os.environ:
            del os.environ["HERMES_SESSION_USER_ID"]
        if context.source.user_name:
            os.environ["HERMES_SESSION_USER_NAME"] = str(context.source.user_name)
        elif "HERMES_SESSION_USER_NAME" in os.environ:
            del os.environ["HERMES_SESSION_USER_NAME"]
