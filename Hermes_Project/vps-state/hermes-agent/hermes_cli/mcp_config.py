...
def _probe_single_server(
    ...
    OAuth servers use a 300s timeout by default so the user has time to
    complete the browser authorization flow and paste the redirect URL.
    Non-OAuth servers use 30s.
    """
    if connect_timeout is None:
        connect_timeout = 300.0 if config.get("auth") == "oauth" else 30.0
