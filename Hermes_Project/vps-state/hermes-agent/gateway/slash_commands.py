...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_vocab_command(self, event: MessageEvent) -> str:
        """Handle /vocab — manage per-user STT vocabulary for voice messages.

        Usage:
          /vocab              — show current vocabulary list
          /vocab add Foo Bar  — add one or more comma/space-separated terms
          /vocab remove Foo   — remove terms
          /vocab clear        — delete all vocabulary
        """
        from tools.user_vocab import load_vocab, add_terms, remove_terms, clear_vocab

        user_id = str(event.source.user_id).strip() if event.source and event.source.user_id else ""
        if not user_id:
            return "⚠️ Could not identify your user ID."
        ...
        if not subcommand or subcommand == "list":
            ...
            if not terms:
                return (
                    "Your STT vocabulary is empty.\n\n"
                    "Add names with: `/vocab add Nishant, DRaaS, Anjali`"
                )
            listed = "\n".join(f"• {t}" for t in terms)
            return f"Your STT vocabulary ({len(terms)} terms):\n{listed}"

        if subcommand == "add":
            raw = sub[1] if len(sub) > 1 else ""
            if not raw.strip():
                return "Usage: `/vocab add Term1, Term2, Term3`"
            new_terms = [t.strip() for t in raw.replace(",", " ").split() if t.strip()]
            updated = add_terms(user_id, new_terms)
            added_str = ", ".join(new_terms)
            return (
                f"✅ Added to your STT vocabulary: {added_str}\n"
                f"Total: {len(updated)} terms. These will be injected on every voice call."
            )

        if subcommand == "remove":
            raw = sub[1] if len(sub) > 1 else ""
            if not raw.strip():
                return "Usage: `/vocab remove Term1, Term2`"
            rm_terms = [t.strip() for t in raw.replace(",", " ").split() if t.strip()]
            updated = remove_terms(user_id, rm_terms)
            rm_str = ", ".join(rm_terms)
            return f"✅ Removed from your STT vocabulary: {rm_str}\nTotal remaining: {len(updated)} terms."

        if subcommand == "clear":
            clear_vocab(user_id)
            return "✅ Your STT vocabulary has been cleared."

        return (
            "Unknown subcommand. Usage:\n"
            "`/vocab` — show list\n"
            "`/vocab add Name1, Name2` — add terms\n"
            "`/vocab remove Name1` — remove terms\n"
            "`/vocab clear` — delete all"
        )
