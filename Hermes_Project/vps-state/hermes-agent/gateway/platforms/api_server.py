...
def _scan_subagent_files(
    ...
    """Best-effort list of files in HERMES_WRITE_SAFE_ROOT created during a subagent's lifetime.

    Scans the safe-write root (default ``/opt/data``, override via
    ``HERMES_WRITE_SAFE_ROOT``) one level deep for files whose mtime
    falls within the child's lifetime window. Capped at 50 results
    so a runaway scan can't block the request.

    The child_session_id parameter is currently unused; kept in the
    signature so a future per-session log path (e.g. tool-call file
    registry) can be slotted in without changing call sites.
    """
    import os as _os
    _ = child_session_id  # reserved for future per-session lookups
    safe_root = _os.environ.get("HERMES_WRITE_SAFE_ROOT", "/opt/data")
    if not _os.path.isdir(safe_root):
        return []
    out: List[str] = []
    try:
        for entry in _os.scandir(safe_root):
            try:
                st = entry.stat(follow_symlinks=False)
            except OSError:
                continue
            if st.st_mtime + 0.5 < started_at or st.st_mtime - 0.5 > ended_at:
                continue
                ...
                try:
                    for sub in _os.scandir(entry.path):
                        try:
                            ...
                            continue
                        if sst.st_mtime + 0.5 < started_at or sst.st_mtime - 0.5 > ended_at:
                            continue
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def _resolve_session_user_id(self, request: "web.Request") -> str:
        """
        Resolve the internal user ID for this API request from identity headers.

        Reads (in priority order):
          X-Hermes-User-Id     — internal user ID directly (use when the caller
                                 already knows the canonical ID)
          X-Hermes-User-Email  — SSO login email; resolved via identity registry
                                 to the internal user ID

        Returns the internal user ID string, or "" if no identity can be
        resolved.  The value is passed to set_session_vars(user_id=...) so that
        HERMES_SESSION_USER_ID is always the internal user ID regardless of which
        channel the request arrives from.
        """
        direct_id = request.headers.get("X-Hermes-User-Id", "").strip()
        if direct_id:
            return direct_id

        email = request.headers.get("X-Hermes-User-Email", "").strip()
        if not email:
            return ""

        try:
            from tools._identity_resolver import resolve_user_id
            resolved = resolve_user_id("email", email)
            if resolved:
                return resolved
        except Exception as exc:
            logger.debug("Could not resolve X-Hermes-User-Email %r: %s", email, exc)
        return ""
    ...
    # ── Open WebUI chat_id mapping (subagent-polling fix) ────────────
    #
    # Open WebUI sends ``X-OpenWebUI-Chat-Id`` on every request — a stable
    # UUID that uniquely identifies a chat thread.  Without translation, the
    # api_server generates a fresh hermes session_id for every request, so a
    # subagent dispatched in request N is parented to a hermes session that
    # no future request will ever reuse (and therefore can never be polled).
    #
    # We translate the chat_id to a stable hermes session via
    # ``chat_session_map`` and feed that session_id into the rest of the
    # request pipeline, so all the existing session-continuity machinery
    # (X-Hermes-Session-Id, _derive_chat_session_id, history replay) keeps
    # working unchanged.

    _OPENWEBUI_CHAT_HEADER = "X-OpenWebUI-Chat-Id"

    def _resolve_openwebui_chat_session_id(
        self, request: "web.Request"
    ) -> Optional[str]:
        """Return the persistent hermes session_id for the Open WebUI chat, or None.

        Reads the ``X-OpenWebUI-Chat-Id`` header and looks up the
        ``chat_session_map`` row.  Creates a new row + session on first sight
        so subsequent requests from the same chat get the same session.
        Returns None when the header is absent (caller falls back to the
        normal ``X-Hermes-Session-Id`` / derived flow).
        """
        chat_id = request.headers.get(self._OPENWEBUI_CHAT_HEADER, "").strip()
        if not chat_id:
            return None
        # Sanitize the same way we do for X-Hermes-Session-Id: reject control
        # characters that could enable header injection downstream.
        if re.search(r"[\r\n\x00]", chat_id):
            return None
        ...
        user_id = ""
        try:
            user_id = self._resolve_session_user_id(request) or ""
        except Exception:
            pass
        try:
            return db.get_or_create_chat_session(chat_id, source="api_server", user_id=user_id)
        except Exception as e:
            logger.debug("chat_session_map lookup failed: %s", e)
            return None

    def _inject_completed_subagent_results(
        self,
        parent_session_id: str,
        conversation_history: List[Dict[str, str]],
        user_message: str,
    ) -> tuple:
        """Prepend completed subagent results to *conversation_history*.

        Returns ``(new_history, n_injected)``.  *n_injected* is the number of
        child-session results actually added (zero means nothing was
        pending).  Idempotent: a child result is only injected once per
        (parent, child) pair, tracked via the ``last_injected_at`` marker
        on the ``chat_session_map`` row.

        Injected shape — a single ``user`` role message at the head of the
        history, formatted as a Hermes system event so the model treats it
        as context for the new turn rather than a user-authored message:

            {
              "role": "user",
              "content": "[Hermes background subagent result — session <id>]\n\n<summary>"
            }

        This deliberately uses ``user`` (not ``tool`` or ``system``) because:
          * tool messages need a matching assistant tool_calls in the
            conversation; without one the model ignores them (verified
            in the pilot smoke test).
          * system messages can be silently dropped by the upstream model
            when the chat-completions endpoint extracts system messages
            into a separate field.
          * a user role message is always preserved in the conversation
            and is the natural shape for a "you just received these
            results, please act on them" injection.

        The ``[Hermes ...]`` prefix lets the model recognise it as a
        system-injected event rather than something the human user wrote
        (mirrors how AGENTS.md describes the async-delegation completion
        queue: "completions surface as a NEW turn when the agent is idle").
        """
        if not parent_session_id or not conversation_history:
            return conversation_history, 0
        db = self._ensure_session_db()
        if db is None:
            return conversation_history, 0
        try:
            last_injected = db.get_chat_session_last_injected(parent_session_id)
        except Exception:
            return conversation_history, 0
        try:
            completed = db.get_completed_subagent_results(
                parent_session_id, since=last_injected
            )
        except Exception as e:
            logger.debug("get_completed_subagent_results failed: %s", e)
            return conversation_history, 0
        if not completed:
            return conversation_history, 0

        new_messages: List[Dict[str, str]] = []
        newest_ended_at = last_injected
        for child in completed:
            child_id = child["id"]
            try:
                summary = (db.get_subagent_final_message(child_id) or "").strip()
            except Exception:
                summary = ""
            # Trim long summaries so the injection doesn't blow the prompt
            # cache for long-lived chats.
            if len(summary) > 4000:
                summary = summary[:4000] + "\n\n[…truncated; full transcript in session " + child_id + "]"

            header = (
                f"[Hermes background subagent result — session {child_id}]\n"
                f"(end_reason={child.get('end_reason')}, "
                f"output_tokens={child.get('output_tokens')}, "
                f"tool_calls={child.get('tool_call_count')}, "
                f"messages={child.get('message_count')})"
            )
            body = summary if summary else "(no assistant summary stored — check session " + child_id + " in `hermes sessions`)"
            new_messages.append({
                "role": "user",
                "content": f"{header}\n\n{body}",
            })
            ended_at = child.get("ended_at")
            if ended_at is not None and float(ended_at) > newest_ended_at:
                newest_ended_at = float(ended_at)

        # Prepend so the model sees results before the user turn.
        new_history = new_messages + list(conversation_history)
        # Update the marker so the next request doesn't re-inject these.
        try:
            db.mark_subagent_results_injected(parent_session_id, newest_ended_at)
        except Exception as e:
            logger.debug("mark_subagent_results_injected failed: %s", e)

        logger.info(
            "api_server: injected %d completed subagent result(s) for parent=%s",
            len(new_messages), parent_session_id,
        )
        return new_history, len(new_messages)
    ...
    async def _handle_gws_auth_callback(self, request: "web.Request") -> "web.Response":
        ...
        The OAuth state parameter encodes "{user_id}|{service_key}" (both set
        server-side by gws_auth.get_auth_url). For backward compatibility, a
        state without "|" is treated as a legacy user_id with service_key="google".
        """
        code = request.rel_url.query.get("code", "")
        state = request.rel_url.query.get("state", "")  # "{user_id}|{service_key}"
        ...
        # Parse composite state: "{user_id}|{service_key}"
        from tools.gws_auth import decode_state
        user_id, service_key = decode_state(state)

        try:
            from tools.gws_auth import exchange_and_store
            exchange_and_store(user_id, code, service_key=service_key)
            logger.info(
                "GWS token stored: user=%s service=%s",
                user_id, service_key,
            )
            # Resolve user_id → display name and Telegram ID for notification.
            # user_id is now the canonical email; _notify_telegram_user needs
            # a numeric Telegram ID, so we look it up from identities.
            try:
                from tools._user_registry import get_user_config
                uconf = get_user_config(user_id)
                display = uconf.get("name") or uconf.get("email") or ("user " + user_id)
                tg_ids = uconf.get("identities", {}).get("telegram", [])
                notify_tg_id = str(tg_ids[0]) if tg_ids else None
            except Exception:
                display = "user " + user_id
                notify_tg_id = None
            account_label = f" ({service_key})" if service_key != "google" else ""
            notify_text = (
                f"Google{account_label} authorized for {display}. "
                "Gmail, Calendar and Drive are now connected. "
                "Close this tab and return to Telegram."
            )
            if notify_tg_id:
                asyncio.create_task(self._notify_telegram_user(notify_tg_id, notify_text))
            return web.Response(
                content_type="text/html",
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        result, usage = await self._run_agent(
            ...
            session_id=session_id,
            gateway_session_key=gateway_session_key,
            user_id=self._resolve_session_user_id(request),
        )
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        session_user_id = self._resolve_session_user_id(request)
        ...
        # Open WebUI sends X-OpenWebUI-Chat-Id with every request — use it as
        # the stable session_id so subagent results from earlier turns in
        # the same chat can be polled and re-injected.  Takes precedence over
        # X-Hermes-Session-Id when both are set.
        if not provided_session_id:
            owui_session_id = self._resolve_openwebui_chat_session_id(request)
            if owui_session_id:
                provided_session_id = owui_session_id
    ...
    async def _run_agent(
        ...
        ``user_id`` is the telegram_id of the authenticated API caller, resolved
        from X-Hermes-User-Email / X-Hermes-User-Id headers by the endpoint
        handler.  It is bound via set_session_vars so tools reading
        HERMES_SESSION_USER_ID (gws_auth, terminal subprocesses, etc.) see the
        ...
        def _run():
            ...
            session_tokens = []
            # ── Inject completed subagent results into the parent's context ──
            # When the api_server has a stable parent_session_id (from
            # X-OpenWebUI-Chat-Id or X-Hermes-Session-Id), surface any
            # delegate_task children that completed since the last turn.
            # This is the core of the subagent-polling fix — without it, a
            # background delegate stays orphaned in state.db and the model
            # keeps claiming "still working" forever.
            effective_history = conversation_history
            try:
                if session_id:
                    effective_history, _n = self._inject_completed_subagent_results(
                        session_id, conversation_history, user_message,
                    )
            except Exception as e:  # never let injection break a request
                logger.debug("subagent result injection skipped: %s", e)
            try:
    ...
    async def connect(self) -> bool:
        ...
        # Pre-flight checks (API key, weak key check, port conflict).
        # Returns False on any hard failure — caller surfaces that to the
        # operator and skips the web-server start.
        if not await self._connect_preflight():
            return False
    ...
    def _register_routes(self) -> None:
        """Register all aiohttp routes on ``self._app``.

        Split out from ``connect()`` so tests can build the app + register
        routes without binding a real socket. Idempotent on a fresh
        ...
        # Subagent polling surface — Open WebUI (or any frontend) can read
        # subagent status for a chat without going through /v1/chat/completions.
        # These complement the automatic result-injection in
        # _run_agent: a frontend that wants to render a "🔀 2" badge or a
        # "research agent X finished" toast hits these endpoints.
        self._app.router.add_get("/v1/chats/{chat_id}/subagents", self._handle_list_subagents)
        self._app.router.add_get("/v1/chats/{chat_id}/subagents/{child_id}", self._handle_get_subagent)
        ...
        # sweep_task is a module-level method that creates a future; we register
        # it in self._background_tasks so disconnect() can drain it on shutdown.
        # Done outside ``connect()`` so the refactored _register_routes can be
        # called from tests without spawning the sweep coroutine.
        if not getattr(self, "_sweep_started", False):
            self._sweep_started = True
            import asyncio as _asyncio
            sweep_task = _asyncio.create_task(self._sweep_orphaned_runs())
            try:
    ...
    async def _connect_preflight(self) -> bool:
        """Run the start-up checks that ``connect()`` used to inline.

        Validates the API key, warns about unsandboxed terminals on public
        binds, and fails fast on port conflicts. Returns True on success,
        False on a hard failure (caller should ``return False`` from
        ``connect()`` without starting the web server).
        """
        import socket as _socket
        from gateway.platforms.api_server import is_network_accessible
    ...
    # ── Subagent polling endpoints (Phase 2) ─────────────────────────
    #
    # These complement the automatic result-injection in _run_agent:
    # a frontend (Open WebUI badge, custom dashboard, mobile app, etc.)
    # can hit these to render subagent status without going through the
    # chat-completions round-trip. Read-only, safe to call frequently.
    #
    # Security: the chat_id in the URL path is the Open WebUI chat
    # identifier (X-OpenWebUI-Chat-Id). Anyone who knows it can read the
    # subagent list for that chat. Same auth gate as the rest of the API
    # surface (API key required when configured).

    async def _handle_list_subagents(self, request: "web.Request") -> "web.Response":
        """GET /v1/chats/{chat_id}/subagents — list subagent runs for a chat.

        Returns up to ``limit`` subagent runs (default 20, max 50) sorted
        by ``started_at`` descending. Each entry includes status, end
        reason, token usage, summary excerpt, files created, and title.

        Optional query params:
          - ``include_full_message=true`` — include the full final
            assistant message text (no 500-char cap).
          - ``include_files=false`` — skip the HERMES_WRITE_SAFE_ROOT
            scan (faster).
        """
        auth_err = self._check_auth(request)
        if auth_err:
        ...
        chat_id = request.match_info["chat_id"].strip()
        if not chat_id or re.search(r"[\r\n\x00]", chat_id):
            return web.json_response(
                {"error": {"message": "Invalid chat_id", "type": "invalid_request_error"}},
                status=400,
            )
        ...
        include_full_message = request.query.get("include_full_message", "").lower() in ("1", "true", "yes")
        include_files = request.query.get("include_files", "true").lower() not in ("0", "false", "no")

        # Look up the parent session via chat_session_map. Unlike the
        # _resolve_openwebui_chat_session_id path used by chat-completions,
        # this is a *read* endpoint — never create a session here.
        db = self._ensure_session_db()
        if db is None:
            return web.json_response(
                {"error": {"message": "Session database unavailable", "code": "session_db_unavailable"}},
                status=503,
            )
        try:
            row = db._conn.execute(
                "SELECT parent_session_id, last_seen_at, last_injected_at FROM chat_session_map WHERE chat_id = ?",
                (chat_id,),
            ).fetchone()
        except Exception as exc:
            return web.json_response(
                {"error": {"message": f"chat_session_map query failed: {exc}"}},
                status=500,
            )
        if not row:
            return web.json_response(
                {"error": {"message": f"Unknown chat_id: {chat_id!r}", "code": "chat_not_found"}},
                status=404,
            )
        ...
        # Read children
        try:
            children_rows = db._conn.execute(
                """
                SELECT id, started_at, ended_at, end_reason, output_tokens,
                       message_count, tool_call_count, title, model
                FROM sessions
            ...
            return web.json_response(
                {"error": {"message": f"child query failed: {exc}"}},
                status=500,
            )
        ...
        for r in children_rows:
            child = dict(r)
            ended = child.get("ended_at")
            child["status"] = "completed" if ended else "running"
            if ended:
                try:
                    ...
                    msg = None
                summary = (msg["content"] if msg else "") or ""
                if include_full_message:
                    child["summary"] = summary
                    child.pop("summary_excerpt", None)
                else:
                    child["summary_excerpt"] = summary[:500]
            else:
                child["summary_excerpt"] = None

            if include_files and ended:
                started = child.get("started_at")
                if started is not None:
                    try:
                        child["files_created"] = _scan_subagent_files(
                            child["id"], started, ended,
                        )
                    except Exception as exc:
                        logger.debug("files_created scan failed: %s", exc)
                        child["files_created"] = []
                else:
                    child["files_created"] = []
            else:
    ...
    async def _handle_get_subagent(self, request: "web.Request") -> "web.Response":
        """GET /v1/chats/{chat_id}/subagents/{child_id} — full child session detail.

        Returns the child session row plus the message history (in OpenAI
        chat-completions format) and the list of files created. Useful
        for a "show me what the research agent found" deep-dive view.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        chat_id = request.match_info["chat_id"].strip()
        child_id = request.match_info["child_id"].strip()
        if not chat_id or not child_id or re.search(r"[\r\n\x00]", chat_id + child_id):
            return web.json_response(
                {"error": {"message": "Invalid chat_id or child_id", "type": "invalid_request_error"}},
                status=400,
            )
        ...
        # Verify chat_id → parent → child linkage (defense in depth — the
        # child_id alone would let anyone enumerate, so we gate on the
        # chat_id being a known mapped chat first).
        try:
            row = db._conn.execute(
        ...
        try:
            child_row = db._conn.execute(
                ...
                SELECT id, started_at, ended_at, end_reason, output_tokens,
                       message_count, tool_call_count, title, model,
                       input_tokens, cache_read_tokens, parent_session_id,
                       source
        ...
        if not child_row or child_row["parent_session_id"] != parent_id or child_row["source"] != "subagent":
            return web.json_response(
                {"error": {"message": f"Subagent not found: {child_id!r}", "code": "subagent_not_found"}},
                status=404,
            )

        child = dict(child_row)
        child["status"] = "completed" if child.get("ended_at") else "running"
        ...
        try:
            ...
            return web.json_response(
                {"error": {"message": f"message query failed: {exc}"}},
                status=500,
            )
        ...
        for m in msg_rows:
            ...
            if isinstance(md.get("content"), str):
                try:
                    parsed = json.loads(md["content"])
                    md["content"] = parsed
                except (TypeError, ValueError):
                    pass
        ...
        if child.get("started_at") and child.get("ended_at"):
            try:
                files_created = _scan_subagent_files(
                    child_id, child["started_at"], child["ended_at"],
                )
            except Exception as exc:
                logger.debug("files_created scan failed: %s", exc)
