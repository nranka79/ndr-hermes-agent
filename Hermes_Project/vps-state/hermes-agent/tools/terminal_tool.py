        ...
        if background:
            ...
            while retry_count <= max_retries:
                try:
                    ...
                    # Always write (even when empty) to prevent stale cross-session contamination:
                    # env.env is shared across all sessions, so a previous session's value must
                    # be explicitly cleared when the current session has no user identity.
                    try:
                        from gateway.session_context import _UNSET, _SESSION_USER_ID
                        _tid = _SESSION_USER_ID.get()
                        if _tid is _UNSET:
                            _tid = ""
                        if hasattr(env, "env"):
