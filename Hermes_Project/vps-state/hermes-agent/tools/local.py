...
def _make_run_env(env: dict) -> dict:
    ...
    # Always inject when not _UNSET (even empty string) so that a prior session's
    # value in env (merged from env.env above) cannot contaminate this request.
    try:
        from gateway.session_context import _UNSET, _VAR_MAP
        for var_name, var in _VAR_MAP.items():
            value = var.get()
            if value is not _UNSET:
                run_env[var_name] = value if value else ""
    except Exception:
        pass
