...
def setup_model_config():
    """Write config.yaml — prefers MiniMax if MINIMAX_API_KEY is set, falls back to OpenRouter."""
    print("\n" + "="*80)
    ...
    if minimax_key:
        ...
        print("\n⚠ Neither MINIMAX_API_KEY nor OPENROUTER_API_KEY set — skipping model config write")
        print("="*80 + "\n")
        return False
    ...
    try:
        ...
        print(f"  - model: {model}")
        print(f"\n✓ {model} (via {provider}) is now the default model!")
        print("="*80 + "\n")
        return True
    except Exception as e:
...
def setup_registry_sheet():
    ...
    TABS = {
        ...
        "vocab_corrections": ["misspelt","correct","category","first_seen","occurrences","notes"],
    }
