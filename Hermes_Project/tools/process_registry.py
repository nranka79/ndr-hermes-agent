    ...
    def spawn_local(
        ...
        if use_pty:
            ...
            try:
                ...
                # Bridge per-request session contextvars into the subprocess
                # env so a background process (gws_auth, gws_vault, …) sees
                # the *current* user's identity, not whatever stale value
                # is sitting in os.environ from process startup or a prior
                # request.  Without this, terminal(background=True) silently
                # routes to the wrong per-user token.  See issue: terminal
                # subprocess identity leak.
                from gateway.session_context import inject_session_context_into_env
                inject_session_context_into_env(pty_env)
        ...
        # Bridge per-request session contextvars into the subprocess env so a
        # background process (gws_auth, gws_vault, …) sees the *current*
        # user's identity, not whatever stale value is sitting in os.environ
        # from process startup or a prior request.  Without this,
        # terminal(background=True) silently routes to the wrong per-user
        # token — the contextvar path resolves to "ndr@draas.com" but the
        # subprocess sees "sales1.blr@draas.com" from a leaked os.environ.
        from gateway.session_context import inject_session_context_into_env
        inject_session_context_into_env(bg_env)
