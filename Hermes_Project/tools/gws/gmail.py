...
Command syntax:
  ...
  gmail messages send --to X --subject Y --body Z [--cc X] [--bcc X] [--bodyHtml HTML]
...
def handle_gmail(parts: list, account_email: str) -> str:
    ...
    if resource == "messages":
        ...
        if action == "send":
            # Flag-based path: --to, --subject, --body, [--cc], [--bcc], [--bodyHtml]
            # This is the recommended way to send a plain email without attachment.
            # The old raw-MIME path (--body '{"raw":"..."}') is preserved as fallback.
            ...
            if to_addr:
                ...
                try:
                    ...
                    return f"Error building message: {e}"
            ...
            # Fallback: caller passed a pre-built --body '{"raw":"..."}'
            body = json_flag(flags, "body", {})
            if not body:
                return "Error: --to is required, or pass --body '{\"raw\":\"<base64url>\"}'"
