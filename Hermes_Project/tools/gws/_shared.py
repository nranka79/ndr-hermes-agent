...
# ---------------------------------------------------------------------------
# Per-user session token (set by n8n_tool before each GWS call)
# ---------------------------------------------------------------------------

_session_gws_token: contextvars.ContextVar = contextvars.ContextVar(
    "session_gws_token", default=None
)


def set_session_gws_token(access_token: str) -> None:
    _session_gws_token.set(access_token)


def build_service_for_session(service: str, version: str):
    """Build a Google API client using the session-scoped per-user token from n8n."""
    try:
        from google.oauth2.credentials import Credentials
        from googleapiclient.discovery import build
    except ImportError:
        raise RuntimeError("google-auth and google-api-python-client required.")
    token = _session_gws_token.get()
    if not token:
        raise RuntimeError(
            "No per-user GWS token in session. "
            "This is a system error — n8n_tool should have set it."
        )
    creds = Credentials(
        token=token,
        expiry=datetime.datetime.utcnow() + datetime.timedelta(seconds=3300),
    )
