        ...
        if background:
            ...
            while retry_count <= max_retries:
                try:
                    ...
                    # Session contextvars (USER_ID, KEY, PLATFORM, …) are
                    # bridged into the subprocess env centrally by the
                    # environment backend's _make_run_env() via
                    # ``gateway.session_context.inject_session_context_into_env``.
                    # No per-tool injection needed here — that path is
                    # exercised for every execute() call regardless of
