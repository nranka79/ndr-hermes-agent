...
# ---------------------------------------------------------------------------
# delegate_status — explicit subagent polling (Phase 2)
# ---------------------------------------------------------------------------
#
# When the parent agent has dispatched background subagents, it sometimes
# needs to *explicitly* check whether they have completed (e.g. before
# responding to a "are the research agents done yet?" prompt). The
# api_server's chat-completions path automatically injects completed
# subagent results on the next turn, but in-process calls (CLI, Telegram,
# Discord) don't get that injection — they have to use this tool.
#
# The output shape mirrors what the api_server's
# ``/v1/chats/{chat_id}/subagents`` endpoint returns so frontends can
# render it directly.
# ---------------------------------------------------------------------------


def _parent_session_id(parent_agent) -> Optional[str]:
    """Return the stable parent session id for the parent_agent.

    Works for both AIAgent (uses ``self.session_id``) and lightweight
    proxy agents that store the id under a different attribute.
    """
...
def _open_session_db_or_none():
    """Return a SessionDB instance, or None if state.db is unavailable.

    Lets ``delegate_status`` degrade gracefully when the parent runs
    outside a normal Hermes process (tests, minimal embeddings, etc.).
    """
    try:
        from hermes_state import SessionDB
        return SessionDB()
    except Exception as exc:  # pragma: no cover — best-effort
        logger.debug("SessionDB unavailable for delegate_status: %s", exc)
        return None
...
def delegate_status(
    ...
    """List recent subagent runs for this conversation, with status and a brief summary.

    Used by the model to explicitly check on background research agents it
    dispatched earlier in the conversation. Companion to ``delegate_task``.

    Parameters
    ----------
    parent_agent
        The AIAgent instance (injected by the tool dispatcher).
    limit
        Max number of subagent runs to return. Capped at 50.
    include_full_message
        If True, include the full final assistant message text for each
        completed subagent. Default False (only a 500-char excerpt) to
        keep tool results from blowing the prompt cache.
    include_files
        If True (default), scan the filesystem for files created during
        each child session's lifetime and include their paths. Uses
        ``HERMES_WRITE_SAFE_ROOT`` (defaults to ``/opt/data``) as the
        scan root.

    Returns
    -------
    JSON
        ``{"parent_session_id": str, "count": int, "subagents": [...]}``.
        Each subagent entry has: ``id``, ``status`` (running/completed),
        ``started_at``, ``ended_at``, ``end_reason``, ``output_tokens``,
        ``message_count``, ``tool_call_count``, ``summary_excerpt`` (or
        ``summary`` if include_full_message=True), ``files_created``
        (list of paths if include_files=True), ``title``.
    """
    if parent_agent is None:
        return json.dumps({"error": "delegate_status requires a parent agent context."})

    parent_id = _parent_session_id(parent_agent)
    if not parent_id:
        return json.dumps({"error": "Parent session has no session_id; cannot enumerate children."})

    db = _open_session_db_or_none()
    if db is None:
        return json.dumps({"error": "SessionDB unavailable; cannot read child sessions."})
    ...
    try:
        ...
        return json.dumps({"error": f"Failed to query child sessions: {exc}"})
    ...
    for r in rows:
        ...
        # Always set files_created for a stable output shape — empty list
        # when the caller opts out or there's nothing to report.
        if include_files and ended:
            started = child.get("started_at")
            if started is not None:
                try:
                    child["files_created"] = _scan_files_created(
                        child_id, started, ended,
                    )
                except Exception as exc:
                    logger.debug("files_created scan failed for %s: %s", child_id, exc)
                    child["files_created"] = []
            else:
                child["files_created"] = []
        else:
...
def _scan_files_created(child_session_id: str, started_at: float, ended_at: float) -> List[str]:
    """Find files in HERMES_WRITE_SAFE_ROOT modified during the child's lifetime.

    Best-effort, capped at 50 results, 1MB stat-batch size, so this never
    blocks the model for more than a few hundred ms even on large
    sandboxes. Returns a list of POSIX paths (str).
    """
    import os as _os
    ...
    out: List[str] = []
    try:
        # Walk top-level only; sandbox subdirs are typically per-task.
        # We do a 1-deep recursion at most so we don't pay for huge trees.
        for entry in _os.scandir(safe_root):
            ...
            if entry.is_file(follow_symlinks=False):
                ...
                # One level deeper, just file names
                try:
...
DELEGATE_STATUS_SCHEMA = {
    ...
    "description": (
        "List the most recent subagent runs (delegate_task children) for this "
        "conversation, with their status (running/completed), end reason, "
        "token cost, and a brief summary excerpt of the final assistant "
        "message. Optionally includes the list of files each completed "
        "subagent created in the safe-write root (/opt/data by default).\n\n"
        "USE THIS WHEN the user asks 'are the research agents done?', "
        "'what did the subagents find?', or anything about background "
        "work that was previously dispatched. Note: the api_server "
        "automatically injects completed subagent results into the next "
        "chat-completions turn via the chat_session_map pipeline, so you "
        "usually do NOT need to call this tool — read the conversation "
        "history first. If the history does not contain the result, "
        "call this tool to surface it.\n\n"
        "Returns JSON: {parent_session_id, count, subagents: [...]}. "
        "Each subagent has: id, status, started_at, ended_at, end_reason, "
        "output_tokens, message_count, tool_call_count, summary_excerpt, "
        "files_created, title."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            "limit": {
                ...
                "maximum": 50,
                "description": (
                    "Max number of subagent runs to return. Default 20, capped at 50."
                ),
            },
            "include_full_message": {
                "type": "boolean",
                "description": (
                    "If true, return the full final assistant message text for each "
                    "completed subagent (no 500-char excerpt cap). Default false."
                ),
            },
            "include_files": {
                "type": "boolean",
                "description": (
                    "If true (default), scan HERMES_WRITE_SAFE_ROOT for files "
                    "created during each subagent's lifetime and include paths."
                ),
            },
        },
    },
...
registry.register(
    ...
    handler=lambda args, **kw: delegate_status(
        ...
        include_full_message=bool(args.get("include_full_message", False)),
