...
def _transcribe_local(
    ...
    *,
    ...
    When *user_id* is provided the caller's stored vocabulary list is
    injected via ``initial_prompt`` and ``hotwords`` so that names and
    domain-specific proper nouns are recognised more accurately.
    """
    ...
    try:
        ...
        # Per-user vocabulary injection via initial_prompt + hotwords.
        if user_id:
            try:
                from tools.user_vocab import load_vocab, build_initial_prompt, build_hotwords
                vocab = load_vocab(user_id)
                if vocab:
                    transcribe_kwargs["initial_prompt"] = build_initial_prompt(vocab)
                    transcribe_kwargs["hotwords"] = build_hotwords(vocab)
                    logger.debug(
                        "STT vocab injection for user %s: %d terms",
                        user_id, len(vocab),
                    )
            except Exception as _vocab_exc:
                logger.debug("Vocab injection failed (non-fatal): %s", _vocab_exc)
...
def transcribe_audio(file_path: str, model: Optional[str] = None) -> Dict[str, Any]:
    ...
    # Resolve the current session's user ID for per-user vocab injection.
    _session_user_id: Optional[str] = None
    try:
        from gateway.session_context import get_session_env
        _session_user_id = get_session_env("HERMES_SESSION_USER_ID", "") or None
    except Exception:
        pass
    ...
    if provider == "local":
        ...
        return _transcribe_local(file_path, model_name, user_id=_session_user_id)
