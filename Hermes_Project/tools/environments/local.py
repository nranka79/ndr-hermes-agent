...
def _make_run_env(env: dict) -> dict:
    ...
    # Bridge per-request session contextvars into the subprocess env so
    # subprocesses see the *current* request's identity (USER_ID, KEY,
    # PLATFORM, …) — not whatever stale value is sitting in os.environ
    # from process startup or a previous request.  The helper is the
    # single source of truth for this bridging; see
    # ``gateway.session_context.inject_session_context_into_env`` for
    # the contract.
    from gateway.session_context import inject_session_context_into_env
    inject_session_context_into_env(run_env)
