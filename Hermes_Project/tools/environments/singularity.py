...
def _get_or_build_sif(image: str, executable: str = "apptainer") -> str:
    ...
    with _sif_build_lock:
        ...
        # Bridge the current gateway request's session contextvars into
        # the SIF-build subprocess env.  This is the same defense-in-depth
        # applied in tools/environments/local.py:_make_run_env and the
        # docker backend: subprocesses see the *current* request's
        # identity, not whatever stale value is in os.environ.  See
        # ``gateway.session_context.inject_session_context_into_env``
        # for the contract.
        try:
            from gateway.session_context import inject_session_context_into_env
            inject_session_context_into_env(env)
        except Exception:
            pass
...
class SingularityEnvironment(BaseEnvironment):
    ...
    def _start_instance(self):
        ...
        # Bridge the current gateway request's session contextvars into
        # the instance-start subprocess env.  The instance's env is what
        # subsequent ``singularity exec`` calls inherit.  See
        # ``gateway.session_context.inject_session_context_into_env``
        # for the contract.  Note that singularity ``exec`` doesn't
        # expose a per-call env-var injection mechanism, so the instance
        # start env is the only practical point to seed the snapshot.
        # For a true per-request fix here, callers would need to switch
        # to ``exec --env KEY=VAL`` per call — see issue TBD.
        try:
            from gateway.session_context import inject_session_context_into_env
            import os as _os
            _start_env = _os.environ.copy()
            inject_session_context_into_env(_start_env)
            instance_env = _start_env
        except Exception:
            instance_env = None

        try:
            run_kwargs = dict(capture_output=True, text=True, timeout=120, stdin=subprocess.DEVNULL)
            if instance_env is not None:
                run_kwargs["env"] = instance_env
