...
class DockerEnvironment(BaseEnvironment):
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # the docker env so the snapshot captured by init_session (and
        # therefore every subsequent execute()) sees the *current*
        # request's identity (USER_ID, KEY, PLATFORM, …), not whatever
        # stale value is sitting in os.environ from process startup or
        # a prior request.  This mirrors the fix in
        # tools/environments/local.py:_make_run_env and
        # tools/local.py:_make_run_env.  The helper is the single
        # source of truth for this bridging.
        try:
            from gateway.session_context import inject_session_context_into_env
            inject_session_context_into_env(exec_env)
        except Exception:
            pass
