...
def _handle_send(args):
    ...
    # "origin" is a special alias meaning "reply to whichever chat triggered this turn".
    # Resolve it early so all downstream logic sees a normal platform:chat_id target.
    if target.strip().lower() == "origin":
        from gateway.session_context import get_session_env as _gse_origin
        _sess_plat = _gse_origin("HERMES_SESSION_PLATFORM", "").strip()
        _sess_chat = _gse_origin("HERMES_SESSION_CHAT_ID", "").strip()
        if _sess_plat and _sess_chat:
            target = f"{_sess_plat}:{_sess_chat}"
        else:
            return tool_error(
                "Cannot resolve 'origin': no active session context (HERMES_SESSION_PLATFORM or "
                "HERMES_SESSION_CHAT_ID not set). Use an explicit target like 'telegram:CHAT_ID'."
            )
