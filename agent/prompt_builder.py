...
WHATSAPP_LINK_GUIDANCE = (
    ...
    "CRITICAL: the whatsapp_link tool only GENERATES a deep link. It does "
    "NOT send or deliver any message to the recipient. NEVER report that a "
    "WhatsApp message was 'sent', 'delivered', or 'sent to <name>' — that is "
    "false. The only honest way to describe the outcome is that you "
    "GENERATED a link the recipient can tap (on a device with WhatsApp) so "
    "their app opens the chat with the text pre-filled. If you actually sent "
    "a real message (via the send_message tool to a paired WhatsApp gateway), "
    "only then may you say it was sent.\n\n"
