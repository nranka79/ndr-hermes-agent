...
### Data the LLM must NEVER generate

Long structured strings — **OAuth URLs, n8n webhook URLs, file-download
endpoints, 3rd-party API tokens, vault service names, internal service
URLs** — must NEVER be generated by the LLM by stringing together pieces
it has read. They either come from a system source of truth (code, env,
config, files, infrastructure) and are surfaced by a tool, or they don't
exist yet and the LLM tells the user so and asks for them.

**Why this is non-negotiable.** Small/fast chat models (e.g.
`deepseek/deepseek-v4-flash`) silently truncate substrings when
transcribing long URLs into prose — observed failure: dropping `google.`
from `apps.googleusercontent.com`, which then 401s at Google's auth
server with `Error 401: invalid_client`. The same class of failure will
hit any other structured token the LLM is asked to retype.

**Canonical implementation: `tools/send_oauth_url.py`.** The tool
computes the OAuth URL via `gws_auth.get_auth_url()` and delivers it via
the right primitive for the current session's channel:

| Channel | Delivery primitive | URL exposed to LLM? |
|---|---|---|
| Telegram | inline-keyboard button | No — Telegram carries the URL byte-for-byte |
| CLI | printed to stderr | No — user copies from their terminal |
| Open WebUI / markdown | pre-formatted `[label](url)` returned in JSON | Yes, but as a pre-built link the LLM is told to embed verbatim |

Apply the same pattern to other data surfaces as they come up (file
download URLs, n8n webhook URLs, vault service name lookups, etc.). The
return value of any such tool is a **status object** (success,
delivery channel, message_id, error) — never the data itself, unless the
data is small/cheap and the channel is markdown-only and a verbatim
"embed this string" instruction is also returned.
